...
class InputValidator:
    ...
    def validate_session_id(session_id: str) -> bool:
        ...
        # Check if it's a valid UUID format or reasonable session ID
        uuid_pattern = re.compile(
            r'^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$',
            re.IGNORECASE
        )
        
        # Allow UUID format or alphanumeric strings up to 50 chars
        return (
            uuid_pattern.match(session_id) or 
            (len(session_id) <= 50 and session_id.replace('-', '').replace('_', '').isalnum())
        )
    ...
    def validate_code_content(code: str, language: str) -> Dict[str, Any]:
        ...
        result["metrics"]["non_empty_lines"] = len([line for line in lines if line.strip()])
        
        # Check for extremely long lines
        max_line_length = max(len(line) for line in lines) if lines else 0
        if max_line_length > 500:
            result["warnings"].append(f"Very long line detected ({max_line_length} characters)")
    ...
    def _validate_python_code(code: str) -> Dict[str, Any]:
        ...
        if 'import os' in code and any(dangerous in code for dangerous in ['system(', 'exec(', 'eval(']):
    ...
    def _validate_javascript_code(code: str) -> Dict[str, Any]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if (line and 
                ...
                not line.startswith(('if', 'for', 'while', 'function', 'const', 'let', 'var', '//'))):
    ...
    def _validate_html_code(code: str) -> Dict[str, Any]:
        ...
        if '<script' in code.lower():
            if any(dangerous in code.lower() for dangerous in ['document.cookie', 'eval(', 'innerhtml']):
    ...
    def validate_chat_request(data: Dict[str, Any]) -> None:
        ...
        # Validate message content
        if not InputValidator.validate_message_content(data['message']):
            raise InvalidRequestException("Invalid message content")
        ...
        if 'stream' in data and not isinstance(data['stream'], bool):
            raise InvalidRequestException("Stream parameter must be boolean")